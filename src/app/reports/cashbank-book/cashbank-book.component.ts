...
@Component({
  selector: 'app-cashbank-book',
  templateUrl: './cashbank-book.component.html',
  styleUrls: ['./cashbank-book.component.css']
})
export class CashbankBookComponent implements OnInit {
  ...
  getcashledgerdata(){
    this.reportLedgerService.getcashledger({user_id:this.localStorageService.getuserId(),company_id:this.localStorageService.getCompanyId()}).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] == true) {
        this.ledgerBankList = data.Ledger.sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
  ...
  async ledgerreportshow(){
    ...
    this.reportLedgerService.ledgerpostreport(this.ledgerreport).subscribe(data => {
      if (data === null || data === undefined) {
          this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
        } else if (data['success'] == true) {
         this.ledgerReportList = data.JournalVoucher;
          ...
          this.ledgerReportList.forEach((element) => {
            this.balancesum = Utils.convertIntoNumber(this.balancesum) + Utils.convertIntoNumber ( Utils.convertIntoNumber(element.debitAmount) - Utils.convertIntoNumber(element.creditAmount));
            if(this.balancesum < 0){
              this.convernumber = Utils.converttocomaawithdecimal(Utils.convertIntoNumber(-1)*Utils.convertIntoNumber(this.balancesum));
            }else{
              this.convernumber = Utils.converttocomaawithdecimal(Utils.convertIntoNumber(this.balancesum));
            }
          });
