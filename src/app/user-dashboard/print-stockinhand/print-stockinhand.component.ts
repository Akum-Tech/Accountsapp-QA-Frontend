...
@Component({
  selector: 'app-print-stockinhand',
  templateUrl: './print-stockinhand.component.html',
  styleUrls: ['./print-stockinhand.component.css']
})
export class PrintStockinhandComponent implements OnInit {
...
getType(value){
  ...
  async ngOnInit() {
    ...
    this.stockinhandprint();
  }

  async stockinhandprint(){  
    // this.stockinhand.invoice_date = moment(this.stockinhand.invoice_date).format('MM-DD-YYYY');
      ...
      await this.journalvoucherService.getstockinhand({voucher_id:params.uid}).subscribe(async(data) => {
        if (data === null || data === undefined) {
          ...
          this.VoucherID = this.stockinhand.uid; 
          this.amountconvert = data.ItemStockVoucher.total_amount;
          // this.ledger= data.ItemStockVoucher.Ledger;
...
Nextvoucherprint(){
    this.salesVoucherService.NextVoucherGet({id:this.VoucherID,type:"itemstockvoucher"}).subscribe(async data => {
    if (data === null || data === undefined) {
      this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
    } else if (data['success'] === true) {
...
Previousvoucherprint(){
  this.salesVoucherService.PreviousVoucherGet({id:this.VoucherID,type:"itemstockvoucher"}).subscribe(async data => {
    if (data === null || data === undefined) {
      this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
    } else if (data['success'] === true) {
        this.stockinhand = data.JournalVoucher;  
