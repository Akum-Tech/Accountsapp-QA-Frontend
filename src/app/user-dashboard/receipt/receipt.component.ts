...
export class ReceiptComponent implements OnInit {
  ...
  constructor(private messagePanelService: MessagePanelService, private localStorageService: LocalStorageService, public globals: Globals, public ledgerService:LedgerService,public receiptvoucherService:ReceiptvoucherService,public itemsService:ItemsService,public router:Router,config: NgbModalConfig, private ModalService: NgbModal,private modalService: BsModalService,private buyplaneService: BuyplaneService, public route:ActivatedRoute,public dialog: MatDialog) {
    this.config = {
      currentPage: 1,
...
//___________________GET SINGLE USER END____________________________________________________________________________
  ...
  ngOnInit() {
    ...
    this.recieptvoucher.financial_year_start =  new Date(this.activecompany.current_period_start).getFullYear();
    this.recieptvoucher.financial_year_end = new Date(this.activecompany.current_period_end).getFullYear();  
    this.getVoucherAll();
  }
  ...
  getVoucherAll() {
    ...
    this.receiptvoucherService.getreceiptVoucher({company_id:this.localStorageService.getCompanyId(),end_year:this.recieptvoucher.financial_year_end,current_year:this.recieptvoucher.financial_year_start, offset: offsetdata, limit: limitdata }).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {
        if(data.RecieptVoucher.length>0){
          this.receiptVoucherList = data.RecieptVoucher;
          this.loader = false;
  ...
  getLedgerData(item){
    this.RecieptVoucher = item;
    this.recieptvoucher = item.uid;
  }
    getType(value){
    if(value>0){
  ...
  gotoCancel(data){
    this.receiptvoucherService.cancelVoucher(data).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {
        this.cancelData = {};
  ...
  gotodDelete(data){
    this.receiptvoucherService.deleteVoucher(data).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {          
        this.deleteData = {};
