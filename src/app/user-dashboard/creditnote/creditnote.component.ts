...
//___________________GET SINGLE USER END____________________________________________________________________________
  ...
  async ngOnInit() {
    ...
    this.creditVoucher.financial_year_start =  new Date(this.activecompany.current_period_start).getFullYear();
    this.creditVoucher.financial_year_end = new Date(this.activecompany.current_period_end).getFullYear(); 
    this.getVoucherAll();
  }
  ...
  getVoucherAll() {
    ...
    this.creditnoteService.getcreditVoucher({company_id:this.localStorageService.getCompanyId(),end_year:this.creditVoucher.financial_year_end,current_year:this.creditVoucher.financial_year_start, offset: offsetdata, limit: limitdata}).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {
        if(data.CreditVoucher.length>0){
          this.creditinvoiselist = data.CreditVoucher;
          this.loader = false;
  ...
  getLedgerData(item){
    this.CreditVoucher = item;
    this.creditVoucher = item.uid;
    this.showcreditvoucher = true;
  }
  ...
  gotoCancel(data){
    this.creditnoteService.cancelVoucher(data).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {
        this.cancelData = {};
  ...
  gotodDelete(data){
    this.creditnoteService.deleteVoucher(data).subscribe(data => {
      if (data === null || data === undefined) {
        this.messagePanelService.ShowPopupMessageWithLocalization('An error occured, please try again later', this.globals.messageCloseTime, this.globals.messageType.error);
      } else if (data['success'] === true) {  
        this.deleteData = {};
