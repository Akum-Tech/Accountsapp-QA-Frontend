...
export class PaymentvoucherService {
  ...
  createPaymenrvoucher(body): Observable<any> {
    let endpoint = 'api/paymentVoucher';
  ...
  LastDate(body): Observable<any> {
    let endpoint = 'api/paymentVoucher/lastdate';
    return this.http.post<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

  getpaymentVoucher(body): Observable<any> {
    let endpoint = 'api/paymentVoucher/all';
    return this.http.post<any>(this.apiUrl + endpoint, body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
  
  editPaymentVoucher(body): Observable<any> {
    let endpoint = 'api/paymentVoucher/'+body.uid;
    return this.http.put<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

  cancelVoucher(body): Observable<any> {
    let endpoint = 'api/paymentVoucher/cancel/'+body.uid;
    return this.http.put<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
