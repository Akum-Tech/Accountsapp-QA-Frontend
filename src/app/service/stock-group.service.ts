...
export class StockGroupService {
  ...
  stockgroup(body): Observable<any> {
    let endpoint = 'api/stockGroup';
  ...
  putstockgroup(id,body): Observable<any> {
    let endpoint = 'api/stockGroup/'+id;
    return this.http.put<any>(this.apiUrl + endpoint,body,this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
 
  deleteStockGroup(body): Observable<any> {
    let endpoint = 'api/stockGroup/'+body.uid;
