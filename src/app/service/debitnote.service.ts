...
export class DebitnoteService {
  ...
  DebitVoucher(body): Observable<any> {
    let endpoint = 'api/debitVoucher';
  ...
  LastDate(body): Observable<any> {
    let endpoint = 'api/debitVoucher/lastdate';
    return this.http.post<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
  
  getDebitVoucher(body): Observable<any> {
    let endpoint = 'api/debitVoucher/all';
    return this.http.post<any>(this.apiUrl + endpoint, body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

  editdebitVoucher(body): Observable<any> {
    let endpoint = 'api/debitVoucher/'+body.uid;
    return this.http.put<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
  
  getPurpose(body): Observable<any> {
    let endpoint = 'api/purpose/debit_note';
  ...
  cancelVoucher(body): Observable<any> {
    let endpoint = 'api/debitVoucher/cancel/'+body.uid;
    return this.http.put<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
