...
export class ItemsService {
  ...
  getitems(body): Observable<any> {
    let endpoint = 'api/item/all';
  ...
  putitems(id,body): Observable<any> {
    let endpoint = 'api/item/'+id;
    return this.http.put<any>(this.apiUrl + endpoint,body,this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

  deleteitems(body): Observable<any> {
    let endpoint = 'api/item/'+body.uid;
  ...
  getstockgroup(company_id): Observable<any> {
    let endpoint = 'api/stockGroup/all/'+company_id;
    return this.http.get<any>(this.apiUrl + endpoint, this.getHeader())
      .pipe(map(res =>  JSON.parse(res)), catchError(err => this.handleError(err)));
  }
