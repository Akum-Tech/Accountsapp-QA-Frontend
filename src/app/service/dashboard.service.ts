...
export class DashboardService {
  ...
  subLogin(data): Observable<any> {
console.log(data);
let endpoint = 'api/user/addsubUser';
return this.http.post<any>(this.apiUrl + endpoint, data, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
// return this.http.post<any>(this.apiUrl + 'api/v1/subLogin', data,);

  }

  checkUser(data):Observable<any>{
    console.log(data)
    let endpoint='api/user/checkExistUser';
    return this.http.post<any>(this.apiUrl + endpoint, data, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
