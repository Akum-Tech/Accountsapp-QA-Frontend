...
export class StockReportsService {
  ...
  manualstockadd(body): Observable<any> {
    let endpoint = 'api/manualStock/';
  ...
  manualstockput(id,body): Observable<any> {
    let endpoint =  'api/manualStock/'+id;
    return this.http.put<any>(this.apiUrl + endpoint,body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

  manualstockdelete(body): Observable<any> {
    let endpoint = 'api/manualStock/'+body.uid;
    return this.http.delete<any>(this.apiUrl + endpoint, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }

getmanualstoclall(body): Observable<any> {
    let endpoint = 'api/manualStock/all';
    return this.http.post<any>(this.apiUrl + endpoint, body, this.getHeader())
      .pipe(map(res => JSON.parse(res)), catchError(err => this.handleError(err)));
  }
